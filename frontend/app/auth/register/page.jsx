...
function StrengthBar({pw}){
  ...
  const cols=['','#ef4444','#f59e0b','#fbbf24','#10b981'];
  const lbls=['','Weak','Fair','Good','Strong'];
  return pw?(
    <div className="space-y-1">
      <div className="flex gap-1">{[1,2,3,4].map(i=><div key={i} className="h-1 flex-1 rounded-full transition-all duration-300" style={{background:i<=score?cols[score]:'rgba(255,255,255,0.08)'}}/>)}</div>
      <p className="text-xs" style={{color:cols[score]||'#475569'}}>{lbls[score]}</p>
    </div>
  ):null;
}
...
export default function RegisterPage(){
  ...
  const handleSubmit=async ev=>{
    ...
    try{const data=await api.register(form.email,form.password,form.name);saveAuth(data.access_token,data.user);router.push('/dashboard');}
    catch(err){
      if(err.status===409)setErrors({email:'This email already has an account. Sign in instead.'});
  ...
  return(
    <AuthLayout>
      ...
      <p className="text-sm mb-7" style={{color:'#64748b'}}>
        Already registered?{' '}
        <Link href="/auth/login" className="font-semibold underline underline-offset-2" style={{color:'#60a5fa'}}>Sign in</Link>
      </p>
      ...
      <p className="text-xs text-center mt-5" style={{color:'#334155'}}>One account per email · No credit card required</p>
    </AuthLayout>
  );
}
