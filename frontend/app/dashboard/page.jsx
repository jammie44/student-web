...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    setReady(true);
    api.me().then(d=>{ setUser(d); localStorage.setItem('studyhub_user',JSON.stringify(d)); })
      .catch(()=>{ clearAuth(); router.replace('/auth/login'); });
    api.getChats().then(d=>setChats(d.chats||[])).catch(()=>{}).finally(()=>setChatsLoading(false));
  }, [router]);
  ...
  const handleToolSelect = async (toolId) => {
    ...
    try {
      ...
      setChats(prev=>[d.chat,...prev]); setActiveChat(d.chat);
    } catch {}
  };

  const handleSelectChat = (chat) => { setActiveChat(chat); setActiveTool(chat.tool); setMobileSidebarOpen(false); };
  ...
  const updateChatTitle = (chatId, title) => setChats(prev=>prev.map(c=>c.id===chatId?{...c,title}:c));
    ...
    <div className="h-screen flex overflow-hidden bg-ink-950">
      ...
      <div className="hidden md:flex h-full flex-shrink-0">
        <Sidebar chats={chats} loadingChats={chatsLoading} activeChatId={activeChat?.id} activeTool={activeTool}
          ...
          collapsed={!sidebarOpen} user={user} onLogout={handleLogout}/>
      </div>

      <div className={cn('fixed left-0 top-0 h-full z-50 md:hidden transition-transform duration-300 flex-shrink-0',mobileSidebarOpen?'translate-x-0':'-translate-x-full')}>
        <Sidebar chats={chats} loadingChats={chatsLoading} activeChatId={activeChat?.id} activeTool={activeTool}
          onSelectChat={handleSelectChat} onToolSelect={handleToolSelect} onDeleteChat={handleDeleteChat}
          collapsed={false} user={user} onLogout={handleLogout}/>
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full">
        <header className="flex items-center gap-3 px-4 py-3 border-b border-ink-800/60 bg-ink-950/90 backdrop-blur-sm flex-shrink-0">
          ...
          <button onClick={()=>setSidebarOpen(p=>!p)} className="hidden md:flex text-ink-500 hover:text-ink-300 p-2 rounded-lg hover:bg-ink-800 transition-colors">{sidebarOpen?'◀':'▶'}</button>
          <div className="flex-1"/>
          {user?.is_admin && <Link href="/admin" className="hidden sm:flex items-center gap-1.5 text-xs text-ink-500 hover:text-gold-400 transition-colors px-2 py-1 rounded-lg hover:bg-gold-500/10"><span>⚙️</span><span>Admin</span></Link>}
          {user && (
