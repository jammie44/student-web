...
export function Sidebar({ chats, loadingChats, activeChatId, activeTool, onSelectChat, onToolSelect, onDeleteChat, collapsed, user, onLogout }) {
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        <div className="px-3 py-3">
          ...
          {loadingChats ? (
            ...
            filtered.length===0 ? <p className="text-xs text-ink-600 text-center py-4">No chats found</p>
            ...
            Object.entries(grouped).map(([tid,tChats])=>{
              ...
              return (
                <div key={tid} className="mb-4">
                  <p className="text-[10px] font-bold text-ink-600 uppercase tracking-widest px-2 mb-1.5 flex items-center gap-1.5"><span>{tool.icon}</span>{tool.label}</p>
                  <div className="space-y-0.5">{tChats.map(c=><ChatItem key={c.id} chat={c} activeChatId={activeChatId} onSelect={onSelectChat} onDelete={handleDelete} confirm={deleteConfirm}/>)}</div>
                </div>
              );
            })
          )}
          {!loadingChats&&chats.length===0&&(
            <div className="text-center py-8"><p className="text-3xl mb-2">💬</p><p className="text-xs text-ink-600">No chats yet.<br/>Select a tool to start!</p></div>
          )}
        </div>
      </div>
      <div className="px-3 py-3 border-t border-ink-800/50">
        {user?.is_admin && (
          <Link href="/admin" className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-semibold text-ink-400 hover:text-gold-400 hover:bg-gold-500/10 transition-all mb-1">
            <span>⚙️</span>Admin Dashboard
          </Link>
        )}
        <div className="flex items-center gap-3 px-3 py-2 rounded-xl bg-ink-900/40">
          <div className="w-7 h-7 rounded-full bg-gold-500/20 border border-gold-500/30 flex items-center justify-center text-xs font-bold text-gold-400 flex-shrink-0">
            {user?.name?.[0]?.toUpperCase()||user?.email?.[0]?.toUpperCase()||'?'}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-ink-200 truncate">{user?.name||'Student'}</p>
            <p className="text-[10px] text-ink-600 truncate">{user?.email}</p>
          </div>
          <button onClick={onLogout} title="Sign out" className="text-ink-600 hover:text-coral-400 transition-colors text-sm p-1">⇥</button>
        </div>
      </div>
    </aside>
  );
...
function ChatItem({chat,activeChatId,onSelect,onDelete,confirm}){
  ...
  return(
    <div onClick={()=>onSelect(chat)} className={cn('group relative flex items-start gap-2 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-150',activeChatId===chat.id?'bg-gold-500/10 border border-gold-500/20':'hover:bg-ink-800/60 border border-transparent')}>
      ...
      <button onClick={e=>onDelete(e,chat.id)} className={cn('flex-shrink-0 w-5 h-5 flex items-center justify-center rounded text-[10px] transition-all opacity-0 group-hover:opacity-100',confirm===chat.id?'opacity-100 bg-coral-500/20 text-coral-400':'text-ink-600 hover:text-coral-400')}>
        {confirm===chat.id?'✕':'×'}
      </button>
    </div>
  );
