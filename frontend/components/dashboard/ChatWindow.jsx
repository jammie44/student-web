...
export function ChatWindow({ chat, onTitleUpdate, apiBase }) {
  const API = apiBase || process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
  ...
  const loadMessages = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API}/api/chats/${chat.id}/messages`, { headers: getHeaders() });
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-1 min-h-0">
        ...
        {error && (
          <div className="mx-auto max-w-md p-3 bg-coral-500/10 border border-coral-500/30 rounded-xl text-coral-300 text-xs text-center">{error}</div>
        )}
        <div ref={bottomRef} />
      </div>
