...
export function ChatWindow({ chat, onTitleUpdate, onMessageSent }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (onMessageSent) onMessageSent(); // refresh credits
    } catch (err) {
  ...
  const colorCls =
    tool.color === 'gold'  ? 'bg-gold-500/10 border-gold-500/20' :
    tool.color === 'jade'  ? 'bg-jade-500/10 border-jade-500/20' :
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-6 py-4 border-b border-ink-800/60 bg-ink-950/80 backdrop-blur-sm flex-shrink-0">
        <div className={cn('w-9 h-9 rounded-xl flex items-center justify-center text-lg border', colorCls)}>
          {tool.icon}
        </div>
        <div>
          <h2 className="text-sm font-semibold text-ink-100">{chat.title}</h2>
          <p className="text-xs text-ink-500">{tool.label} · powered by Claude AI</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-1 min-h-0">
        ...
        {error && (
          <div className="mx-auto max-w-md p-3 bg-coral-500/10 border border-coral-500/30 rounded-xl text-coral-300 text-xs text-center">
            {error}
          </div>
        )}
        <div ref={bottomRef} />
          ...
          <div className="flex items-end gap-3 bg-ink-900/80 border border-ink-700 rounded-2xl px-4 py-3 focus-within:border-gold-500/50 focus-within:ring-1 focus-within:ring-gold-500/20 transition-all">
            ...
            <button
              ...
              className={cn(
                ...
                input.trim() && !sending
                  ? 'bg-gold-500 hover:bg-gold-400 text-ink-950 shadow-lg shadow-gold-500/20 active:scale-95'
                  : 'bg-ink-800 text-ink-600 cursor-not-allowed'
              )}
            >
              {sending
                ? <div className="w-4 h-4 border-2 border-ink-600 border-t-ink-300 rounded-full animate-spin" />
                : '↑'}
            </button>
          </div>
  ...
  return (
    <div className={cn('flex gap-3 max-w-3xl mx-auto py-1', isUser && 'flex-row-reverse')}>
      ...
      <div className={cn(
        ...
        isUser
          ? 'bg-gold-500/10 border border-gold-500/20 text-ink-100 rounded-tr-sm'
          : 'bg-ink-800/80 border border-ink-700/60 text-ink-200 rounded-tl-sm'
      )}>
        {isUser
...
function TypingIndicator() {
  return (
    <div className="flex gap-3 max-w-3xl mx-auto py-1">
      ...
      <div className="bg-ink-800/80 border border-ink-700/60 rounded-2xl rounded-tl-sm px-4 py-3">
        <LoadingDots />
      </div>
    </div>
  );
...
function EmptyState({ tool, onSend }) {
  const suggestions = {
    ...
    plagiarism: [
      'Paste your essay text here to check originality',
      'Check this paragraph for plagiarism',
    ],
    cv_generator: [
      'Create a CV for a cybersecurity professional with 3 years experience',
      'Write a CV for a Computer Science graduate seeking software engineering roles',
      'Generate a CV for a data scientist with Python and ML skills',
    ],
    assignment: [
      'Format and improve my biology assignment on cell division',
      'Help me structure my essay on climate change',
    ],
    research: [
      'Summarise this research paper on machine learning',
      'Extract key findings from this article',
    ],
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center px-4 py-16">
      <div className={cn('w-16 h-16 rounded-2xl flex items-center justify-center text-3xl mb-4 border', colorCls)}>
        {tool.icon}
      </div>
      <h3 className="font-display font-bold text-ink-200 text-lg mb-2">{tool.label}</h3>
      <p className="text-ink-500 text-sm mb-2 max-w-sm">{tool.description}</p>
      <p className="text-xs text-ink-600 mb-8">Powered by Claude AI · Responses are real and specific to your input</p>
      <div className="space-y-2 w-full max-w-sm">
        <p className="text-[10px] text-ink-600 uppercase tracking-widest font-bold mb-3">Try asking</p>
        {tips.map((tip, i) => (
          <button
            key={i} onClick={() => onSend(tip)}
