  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-6 py-4 border-b border-ink-800/60 bg-ink-950/80 backdrop-blur-sm flex-shrink-0">
        ...
        <div><h2 className="text-sm font-semibold text-ink-100">{chat.title}</h2><p className="text-xs text-ink-500">{tool.label}</p></div>
      </div>
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-1 min-h-0">
        {loading ? <div className="flex items-center justify-center h-40"><Spinner/></div>
         : messages.length===0 ? <EmptyState tool={tool} onSend={msg=>{setInput(msg);textareaRef.current?.focus();}}/>
         : <>{messages.map(m=><MessageBubble key={m.id} message={m}/>)}{sending&&<TypingIndicator/>}</>}
      ...
      <div className="px-4 py-4 border-t border-ink-800/60 bg-ink-950/80 backdrop-blur-sm flex-shrink-0">
        <div className="max-w-3xl mx-auto">
          <div className="flex items-end gap-3 bg-ink-900/80 border border-ink-700 rounded-2xl px-4 py-3 focus-within:border-gold-500/50 focus-within:ring-1 focus-within:ring-gold-500/20 transition-all">
            <textarea ref={textareaRef} rows={1} value={input} onChange={e=>{setInput(e.target.value);autoResize(e);}}
            ...
            <button onClick={handleSend} disabled={!input.trim()||sending}
              className={cn('flex-shrink-0 w-9 h-9 rounded-xl flex items-center justify-center transition-all',
                input.trim()&&!sending?'bg-gold-500 hover:bg-gold-400 text-ink-950 shadow-lg active:scale-95':'bg-ink-800 text-ink-600 cursor-not-allowed')}>
...
function MessageBubble({message}){
  ...
  return(
    <div className={cn('flex gap-3 max-w-3xl mx-auto py-1',isUser&&'flex-row-reverse')}>
      <div className={cn('w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5',isUser?'bg-gold-500/20 border border-gold-500/30 text-gold-400':'bg-ink-800 border border-ink-700 text-ink-400')}>{isUser?'U':'🎓'}</div>
      <div className={cn('flex-1 max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed',isUser?'bg-gold-500/10 border border-gold-500/20 text-ink-100 rounded-tr-sm':'bg-ink-800/80 border border-ink-700/60 text-ink-200 rounded-tl-sm')}>
        {isUser?<p className="whitespace-pre-wrap">{message.content}</p>:<div className="message-content" dangerouslySetInnerHTML={{__html:formatMarkdown(message.content)}}/>}
      </div>
    </div>
  );
}
function TypingIndicator(){
  return(
    <div className="flex gap-3 max-w-3xl mx-auto py-1">
...
function EmptyState({tool,onSend}){
  const tips={study_assistant:['Explain photosynthesis simply','Help me study for exams'],plagiarism:['Check my essay for plagiarism'],cv_generator:['Create a CV for a CS graduate'],assignment:['Format my biology assignment'],research:['Summarise this research paper']};
  const t=tips[tool.id]||tips.study_assistant;
  const cc=tool.color==='gold'?'bg-gold-500/10 border-gold-500/20':tool.color==='jade'?'bg-jade-500/10 border-jade-500/20':'bg-coral-500/10 border-coral-500/20';
  return(
    <div className="flex flex-col items-center justify-center h-full text-center px-4 py-16">
      <div className={cn('w-16 h-16 rounded-2xl flex items-center justify-center text-3xl mb-4 border',cc)}>{tool.icon}</div>
      ...
      <div className="space-y-2 w-full max-w-sm">
        ...
        {t.map((tip,i)=><button key={i} onClick={()=>onSend(tip)} className="w-full text-left px-4 py-3 rounded-xl bg-ink-900/60 border border-ink-800 text-xs text-ink-400 hover:text-ink-200 hover:border-gold-500/30 hover:bg-gold-500/5 transition-all">"{tip}"</button>)}
      </div>
    </div>
  );
}
